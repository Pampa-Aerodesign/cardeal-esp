...
void taskPitot(void* pvParameters){
  ESP_LOGI(PITOTTAG, "Initializing Pitot");
  ...
  DataPacket* datapacket = (DataPacket*)pvParameters;
  ...
  while(1){
    ...
    i2c_master_stop(cmd);
    i2c_master_cmd_begin(I2C_NUM_1, cmd, pdMS_TO_TICKS(1000));
    ...
    // Converting to speed
    float v_ms = (float)(sqrt((pres_psi*13789.5144)/1.225));
    //v_kmh = v_ms*3.6;
    ...
    datapacket->pitot_psi = pres_psi;
    datapacket->pitot_temp = temp_c;
    datapacket->airspeed_ms = v_ms;
